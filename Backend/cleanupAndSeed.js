...
// Blogs to delete (test/fake blogs)
const testTitles = [
  'fake blog for testing',
];
...
async function cleanupAndSeed() {
  try {
    ...
    // Step 1: Delete test/fake blogs
    console.log('\n--- Cleaning up test blogs ---');
    for (const title of testTitles) {
      const result = await Blog.deleteMany({ title: { $regex: title, $options: 'i' } });
      console.log(`  Deleted ${result.deletedCount} blog(s) matching "${title}"`);
    }

    // Also delete any blogs with "fake" or "test" in the title
    const fakeResult = await Blog.deleteMany({
      $or: [
        { title: { $regex: /fake/i } },
        { title: { $regex: /test/i } },
        { title: { $regex: /lorem/i } },
    ...
    console.log(`  Deleted ${fakeResult.deletedCount} additional fake/test blog(s)`);

    // Also remove fake users
    const fakeUserResult = await User.deleteMany({
      $or: [
        { 'personal_info.email': { $regex: /fake/i } },
        { 'personal_info.username': { $regex: /fake/i } },
      ]
    });
    console.log(`  Deleted ${fakeUserResult.deletedCount} fake user(s)`);

    // Step 2: Seed dog-related blogs
    console.log('\n--- Seeding dog-related blogs ---');
    let user = await User.findOne();
    if (!user) {
      console.log('⚠ No user found. Blogs need an author. Please create a user first.');
      await mongoose.disconnect();
      return;
    }

    console.log(`  Using author: ${user.personal_info?.email || user.email || 'unknown'}`);
    ...
    for (const post of blogPosts) {
      ...
      const blog = new Blog({
        ...
        draft: false,
        activity: {
          total_likes: Math.floor(Math.random() * 100) + 20,
          total_comments: Math.floor(Math.random() * 30),
          total_reads: Math.floor(Math.random() * 500) + 100,
    ...
    // Show final blog count
    const totalBlogs = await Blog.countDocuments({ draft: false });
    console.log(`  Total published blogs in DB: ${totalBlogs}`);
